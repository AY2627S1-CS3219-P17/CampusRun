...
- **Routing.** `/api/users/...` goes to user-service and `/api/suppliers/...` to supplier-service. nginx strips the prefix, so each service defines its routes relative to its own root and doesn't repeat its name: `GET /api/suppliers/{id}` is `GET /{id}` in supplier-service.
