...
async def test_invalid_tokens_are_401(client, token):
    response = await client.get("/suppliers", headers={"Authorization": f"Bearer {token}"})
    assert response.status_code == 401


async def test_expired_token_says_session_expired(client):
    response = await client.get("/suppliers", headers={"Authorization": f"Bearer {make_token(expires_in=-60)}"})
    assert response.status_code == 401
...
async def test_student_can_read(client):
    assert (await client.get("/suppliers", headers=STUDENT)).status_code == 200
    assert (await client.get("/meta", headers=STUDENT)).status_code == 200


@pytest.mark.parametrize("path", ["/suppliers", "/delivery-locations"])
...
async def test_student_cannot_edit_or_delete(client):
    created = (await client.post("/suppliers", headers=ADMIN, json=supplier_body())).json()
...
async def test_admin_identity_is_recorded(client):
    ...
    created = (await client.post("/suppliers", headers=headers, json=supplier_body())).json()
